...
to use `llvm-config-8 --bindir` and add this directory as a
prefix to the LLVM binaries.

We can also use `llvm-config-8` to generate the necessary command
line options for `g++` or `clang++`. `clang++` is recommended for
this homework.

    llvm-config-8 --cxxflags --cppflags --cflags --ldflags --system-libs --libs all 
